...
class VmLibraryCard(models.Model):
    ...
    number = fields.Char('Number', size=256, readonly=True, default='New')
    library_card_type_id = fields.Many2one(
        'vm.library.card.type', 'Card Type', required=True)
    ...
    def create(self, vals):
        sequence = self.env['ir.sequence'].next_by_code('vm.library.card') or 'New'
        vals['number'] = sequence
        res = super(VmLibraryCard, self).create(vals)
        if res.type == 'student':
